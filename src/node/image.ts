...
export class CatImage {
    private constructor(public buff: Buffer, private img: Image, public bitmap: Buffer) { }
    ...
    static async drawText(input: string): Promise<CatImage> {
        ...
        return new CatImage(buff, img, Buffer.from(myImageData.data))
    }
    ...
    static async loadFromPath(path: string): Promise<CatImage> {
        ...
        await fs.writeFile('test.png', Buffer.from(imageData.data))

        return new CatImage(buff, img, Buffer.from(imageData.data))
    }

    public getRows(): Buffer[] {
        const rows: Buffer[] = []
        let pixel: number = 0
        let img_lenght = this.bitmap.length / PRINT_WIDTH 
        for (let line = 0; line < img_lenght; line += 1) {
            rows.push(this.bitmap.subarray(pixel, pixel + PRINT_WIDTH))
            pixel += PRINT_WIDTH
        } 
        return rows
    }
